...
4. OSINT results — per source, and watchlist candidates with match strength and which part matched (OSINT結果)
...
- "No evidence" is not innocence — absence from a list does not lower risk.
...
export async function generateReport(req: ReportRequest, complete: ModelComplete): Promise<ReportOutcome> {
  ...
  const facts = {
    ...
    sources: (req.tool_runs ?? []).map((r) => ({ tool: r.tool, status: r.status, citation: r.citation })),
